...
export function reapplyPrivacyBlur() {
  if (
    localStorage.getItem('9mm_privacy_mode') !==
    '1'
  )
    return;
  const cls = '9mm-pos-mgr-privacy-blur';
 ...
 */
export function injectPosStoreForEvents(
  posStore,
) {
  ...
  for (const id of [
    ...
    if (el)
      el.addEventListener('click', (e) => {
        const ic = e.target.closest(
          '[data-copy-tx]',
        );
        if (ic)
          navigator.clipboard
            .writeText(ic.dataset.copyTx)
            .catch(() => {});
      });
  }

  const sg = document.querySelector(
    '.stat-grid',
  );
  if (sg)
