...
export function injectPositionDeps(deps) {
  ...
  if (deps.enterClosedPosView)
    _enterClosedPosView = deps.enterClosedPosView;
  if (deps.exitClosedPosView) _exitClosedPosView = deps.exitClosedPosView;
  if (deps.isViewingClosedPos)
...
export function renderPosBrowser() {
  ...
  if (g('posManagedOnlyToggle')?.checked)
    all = all.filter((e) => _managedTokenIds.has(String(e.tokenId)));
  if (!g('posClosedToggle')?.checked)
    all = all.filter(
      (e) =>
        !(
          e.liquidity !== undefined &&
          e.liquidity !== null &&
          String(e.liquidity) === '0'
        ),
    );
  ...
  if (!filtered.length) {
    list.innerHTML =
      '<div class="pos-empty"><div class="pos-empty-icon">\u25CB</div><div>' +
      (all.length
...
export function activateSelectedPos() {
  ...
  if (!isPositionManaged(active.tokenId) && _fetchUnmanagedDetails)
    _fetchUnmanagedDetails(active);
  act(
    ACT_ICONS.target,
    'fee',
...
function _refreshManageBadge(active) {
  if (!active) return;
...
function _renderPosRow(e) {
  ...
  const dot = managed
    ...
    : '';
  return (
    `<div class="pos-row ${e.active ? 'active-pos' : ''} ${hl ? 'selected' : ''}" data-pos-idx="${e.index}">` +
