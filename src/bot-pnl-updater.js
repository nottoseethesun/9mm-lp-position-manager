...
function toFloat(amount, decimals) { return Number(amount) / Math.pow(10, decimals); }

/** Calculate the USD value of a V3 position from on-chain amounts. */
function positionValueUsd(p, ps, pr0, pr1) {
  const a = rangeMath.positionAmounts(p.liquidity, ps.tick, p.tickLower, p.tickUpper, ps.decimals0, ps.decimals1);
...
async function addPoolShare(posStats, amounts, position, poolState, ethersLib, provider) {
  try {
...
async function residualValueUsd(deps, ethersLib, provider, position, poolState, price0, price1) {
...
function overridePnlWithRealValues(snap, deps, position, poolState, price0, price1, feesUsd, rUsd) {
...
async function estimateGasCostUsd(provider) { try { const f = await provider.getFeeData(); const c = (f.gasPrice ?? 0n) * 800_000n; const p = await fetchTokenPriceUsd(_WPLS, { dextoolsApiKey: config.DEXTOOLS_API_KEY }); return (Number(c) / 1e18) * p; } catch { return 0; } }
...
async function updatePnlAndStats(deps, poolState, ethersLib) {
  ...
  if (pnlTracker) {
    try {
      ...
      const fees = await readUnclaimedFees(provider, ethersLib, position.tokenId, deps.signer);
      const feesUsd = toFloat(fees.tokensOwed0, poolState.decimals0) * price0 + toFloat(fees.tokensOwed1, poolState.decimals1) * price1;
      ...
      const rUsd = await residualValueUsd(deps, ethersLib, provider, position, poolState, price0, price1);
      ...
      overridePnlWithRealValues(pnlSnapshot, deps, position, poolState, price0, price1, feesUsd, rUsd);
  ...
  if (updateBotState) {
    ...
    await addPoolShare(posStats, amounts, position, poolState, ethersLib, provider);
    updateBotState({ poolState: { price: poolState.price, tick: poolState.tick, decimals0: poolState.decimals0, decimals1: poolState.decimals1 }, positionStats: posStats, ...(pnlSnapshot ? { pnlSnapshot } : {}) });
  }
}

module.exports = {
  toFloat, positionValueUsd, fetchTokenPrices, readUnclaimedFees,
  addPoolShare, residualValueUsd, overridePnlWithRealValues,
  estimateGasCostUsd, actualGasCostUsd, updatePnlAndStats,
};
