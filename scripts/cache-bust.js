 ...
 * ui-tokens.css, and 9mm-pos-mgr.css references in public/index.html, plus
 * the drifting cloud PNGs referenced inside 9mm-pos-mgr.css.
 */
...
/*- ui-tokens.css is regenerated by build-ui-tokens.js on every build, so a
 *  browser holding the previous copy would keep an outdated
 *  --info-dialog-max-h after an operator edits ui-defaults.json. */
// eslint-disable-next-line security/detect-unsafe-regex -- Safe: input is local index.html, not user-supplied
h = h.replace(/ui-tokens\.css(\?v=[^"']*)?"/, "ui-tokens.css?v=" + v + '"');
